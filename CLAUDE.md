...
`01-MOCs`, `02-Zettelkasten`, `Incidents` and `Assets` are published with Quartz to `https://atabek04.github.io/knowledge-base` on every push to `main` (`.github/workflows/publish.yml`). Config lives in `site/` (see `site/README.md`); preview with `.\scripts\site-preview.ps1`.
