...
### Two opposite ways a rewrite can fail
...
The point $(2, 1)$ satisfies it — $2(1) = 2$ — and could not have failed to.

#### The principle, in plain words

Picture a balanced scale holding $x_1 + x_2$ against $3$, and a second balanced scale holding $x_1 - x_2$ against $1$. Remove the second scale's contents from the first, left pan from left pan and right pan from right pan. The amounts removed were equal to each other, so <mark style="background: #ABF7F7A6;">removing equal amounts from both sides of a balanced scale leaves it balanced.</mark>

Symbolically, the rule usually stated as "subtracting equals from equals gives equals":

$$\text{if } a = b \text{ and } c = d, \text{ then } a - c = b - d$$

The same holds for adding, and for scaling both sides by a constant.
...
### What losing looks like when it does happen
...
<mark style="background: #ADCCFFA6;">Dividing both sides by a variable is never a legal move on a system — you cannot divide by something that might be zero.</mark>

---

### Derivation does not rule out gaining
...
Notice that squaring <b>passes</b> the losing test — $x = 2$ survives into $x^2 = 4$ intact. The two steps differ only in the second column:
...
### Gaining is not exotic — it happens inside linear systems too
...
`0 = 0` is perfectly true and perfectly derived. But the new system is satisfied by <i>every</i> point on the line $x_1 + x_2 = 3$ — one solution has become infinitely many.

<mark style="background: #FF9E9EA6;">Gaining solutions is one careless multiplication away, which is why a stronger test than derivation is needed.</mark>

That stronger test is [[A rewrite is safe exactly when the move that produced it can be undone|reversibility]].
