...
- **Sharding** — split data across multiple primaries by a key (e.g. user ID, geography). Scales writes, but adds routing logic, cross-shard queries, and a <mark style="background: #FF9E9EA6; font-weight: bold;">hot-shard risk when one key gets a traffic spike</mark> (a crowd in one city → one shard melts).
