...
> A **transformer** is any scikit-learn object that reshapes data through the <mark style="background: #FFF3A3A6;">`fit` / `transform` API</mark>.
...
Because every transformer exposes the same two methods, any of them <mark style="background: #ABF7F7A6;">drops into the same workflow interchangeably</mark> — including a scikit-learn `Pipeline`. You write the steps once; the data flows through each transformer's `fit`/`transform` the same way.
