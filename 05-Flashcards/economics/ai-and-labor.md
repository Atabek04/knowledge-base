TARGET DECK: Tech-KB::Economics::AI and Labor
Tags: economics ai labor
**Chapter:** AI and the Future of Labor
**Related:** [[Economics MOC]]

---

START
Coding Questions
In economic terms, what **is** wealth a claim on?
Back: Wealth is a **claim on other people's time and labor** — the right to direct work toward what you want.
- The money/asset is just the token
- Almost everything you consume is the end of a long **chain of human work**
- Money lets you pull on that chain
Tags: economics ai labor
END

START
Coding Questions
Why is "wealth is a claim on labor" the **premise** for AI-economy predictions?
Back: Because if wealth is **stored labor**, then anything that changes the **cost of labor** changes what wealth itself is.
- When machines supply labor cheaply, a claim on *human* labor loses its obvious value
- Every wage/inequality prediction is a consequence of this definition
Tags: economics ai labor
END

START
Coding Questions
What is the **mechanism** by which AGI drives human wages toward zero?
Back: A wage equals the **marginal productivity** of labor.
- If AGI does the same task at **near-zero marginal cost**, the market value of a human hour falls toward that floor
- So as AGI labor approaches zero cost, human wages decline toward zero
Tags: economics ai labor
END

START
Coding Questions
When wages collapse under AGI, what happens to **labor's share of GDP**?
Back: It **converges toward zero**.
- Total output keeps growing, but a shrinking slice flows to people who work
- The surplus accrues to whoever **owns the machines**
Tags: economics ai labor
END

START
Coding Questions
Why is "AGI drives wages to zero" a **disputed** claim, not a fact?
Back: It is the conclusion of a **model**, not an observation.
- It assumes AGI is a **near-perfect substitute** for human labor across most tasks
- Critics (e.g. the compute-cost argument) reject that assumption
- As of 2026 it is **not** an observed reality
Tags: economics ai labor
END

START
Coding Questions
State the **post-AGI demand paradox**.
Back: AGI lets firms **produce more, cheaper** — but if wages collapse, **fewer consumers can afford to buy**.
- Cheap supply is useless without demand
- The market loop (paychecks → purchases) breaks when the wage link breaks
Tags: economics ai labor
END

START
Coding Questions
In a post-AGI economy, what becomes the **scarce resource**, and how does it reprice human work?
Back: The scarce input becomes **compute** (and the energy feeding it), not intelligence.
- Human work is then valued at **what it would cost to replace it with compute**
- Expensive-to-automate tasks keep value; trivially automated ones track the falling cost of chips
Tags: economics ai labor
END

START
Coding Questions
What reframing does "work is priced at its compute-replacement cost" produce?
Back: The question shifts from *"**Can** AI do my job?"* to *"**Is it worth the compute** to do my job?"*
- This leads directly to the bottleneck argument
Tags: economics ai labor
END

START
Coding Questions
What is Restrepo's **counter-intuitive** claim about which jobs AGI will automate?
Back: Most jobs **won't** be automated — not because AI can't, but because they **aren't worth spending scarce compute on**.
- Survival comes from **neglect**, not human superiority
Tags: economics ai labor
END

START
Coding Questions
Distinguish **bottleneck** work from **supplementary** work (Restrepo).
Back: **Bottleneck** work gates future growth — energy, infrastructure, science, defense, existential-risk reduction → gets the compute.
- **Supplementary** work is everything else — arts, hospitality, support, much research → survives un-automated
Tags: economics ai labor
END

START
Coding Questions
What is the **"sting in the tail"** of jobs surviving by neglect?
Back: Survival is not victory — *"your job is safe"* means *"your job isn't worth taking,"* not *"you're irreplaceable."*
- Wages still **decouple from GDP**; humans keep working while labor's share shrinks
Tags: economics ai labor
END

START
Coding Questions
In a post-AGI world, **who** does power concentrate with, and why?
Back: With the **owners of what stays scarce**: AI models, compute, energy, and land.
- Goods get cheap but wages collapse, so income stops mattering
- Workers lose their bargaining chip (labor); owners control access to essentials
Tags: economics ai labor
END

START
Coding Questions
How does the inequality thesis explain **billionaire behavior** today?
Back: As **positioning** for the AGI divide — securing a spot on the owning side before it hardens.
- Bets on **energy and space** (solar, land, raw materials)
- Massive capital to **own frontier AI infrastructure**
Tags: economics ai labor
END

START
Coding Questions
Under robotic abundance, what gets **cheap** and what stays **expensive**?
Back: **Cheap:** anything reproducible — goods/services fall toward material cost when labor is free.
- **Expensive:** what robots can't print — **land**, fixed-supply raw materials, marginal energy
- Scarcity (and value) **migrates** from labor to these assets
Tags: economics ai labor
END

START
Coding Questions
How does AI change labor's **bargaining lever**?
Back: From **withholding work** (the strike) to **withholding consumption and demand**.
- Strikes only work while production needs humans
- If robots produce, the only chip left is refusing to **buy**
Tags: economics ai labor
END

START
Coding Questions
Why is "withholding consumption" a **weaker** lever than striking?
Back: Strikes are **coordinated and concentrated**; consumption is **diffuse and individual**.
- Organizing millions to *not buy* is far harder than a workforce to *not work*
- This is why **policy**, not bargaining, dominates post-labor proposals
Tags: economics ai labor
END

START
Coding Questions
As of 2026, through which **channel** does AI's labor-market damage show up — and which does it spare?
Back: It shows up in **employment (hiring)**, not in **wages**.
- Adjustment = fewer people hired, not pay cuts for those still working
Tags: economics ai labor
END

START
Coding Questions
What did Stanford's "**Canaries in the Coal Mine**" study find?
Back: Since generative AI took off, workers **aged 22–25** in the most AI-exposed jobs saw a **~16% relative decline in employment**.
- Older and less-exposed workers stayed flat or grew
- Entry-level workers are the **canaries** — first to feel it
Tags: economics ai labor
END

START
Coding Questions
What is the **caveat** on the 2026 entry-level employment findings?
Back: The signal is **early and contested** — *"first inning,"* not settled.
- Anthropic found **no** systematic rise in unemployment for highly exposed workers, only tentative youth hiring slowdown
Tags: economics ai labor
END

START
Coding Questions
Why are **entry-level** roles the most AI-exposed (not senior ones)?
Back: Entry-level work is mostly **codified knowledge** — the "book-learning" and routine implementation that LLMs absorbed and reproduce cheaply.
- Seniors trade in **tacit knowledge**: judgment, context, relationships — not written down to learn from
Tags: economics ai labor
END

START
Coding Questions
What **pipeline trap** does automating entry-level work create?
Back: If juniors can't get hired to do implementation, they never build the **tacit experience** that makes seniors valuable.
- Automating the bottom rung may **starve the pipeline** that produces senior talent
Tags: economics ai labor
END

START
Coding Questions
Contrast AI that **automates** vs AI that **augments** a job.
Back: **Automate** — AI does the task instead of the human → role **shrinks**.
- **Augment** — AI helps the human do it faster → role often **grows**
- Job losses concentrate where AI **substitutes**, stay muted where it **complements**
Tags: economics ai labor
END

START
Coding Questions
How does Anthropic's "**observed exposure**" measure treat automation vs augmentation?
Back: It weights **automated** uses **fully** and **augmenting** uses at **half**.
- Real displacement comes from **substitution**, not assistance
- High theoretical capability displaces no one until used to **replace**, not assist
Tags: economics ai labor
END

START
Coding Questions
What three areas of work give **humans an edge** over AI (Chandar)?
Back: **Physical** tasks (short-to-medium term), **strategic** thinking, and **social** interaction.
- AI is strongest at **implementation**
- ~30% of workers (cooks, mechanics, bartenders) show near-zero AI exposure for these reasons
Tags: economics ai labor
END

START
Coding Questions
Why is the human "physical, strategic, social" advantage **not permanent**?
Back: They are advantages of **timing and cost**, not monopolies.
- The physical edge depends on robotics staying **expensive** — which may not last ("short-to-medium term")
Tags: economics ai labor
END

START
Coding Questions
Contrast a career **ladder** with a career **lattice**.
Back: **Ladder:** one profession, linear rungs, climb for decades.
- **Lattice:** move **sideways** between professions as demand shifts, not just upward
Tags: economics ai labor
END

START
Coding Questions
Why does AI **enable** the career lattice, and what strategy does it imply?
Back: AI lowers the cost of switching fields by acting as a **personalized tutor / on-the-job assistant**.
- Bottleneck shifts from **credentials** to **adaptability**
- Strategy: bet on **learning velocity**, not a single specialization
Tags: economics ai labor
END

START
Coding Questions
What problem does **UBI** answer in a post-labor economy?
Back: If wages collapse but goods still need buyers, UBI puts income in people's hands to **sustain demand**.
- Redistribute AGI's productivity gains as a **regular, unconditional payment**
Tags: economics ai labor
END

START
Coding Questions
List the main **post-labor redistribution variants**.
Back: - **UBI** — cash to all, no strings
- **Universal Basic Services** — guarantee housing/healthcare/food directly
- **Negative income tax** — top up incomes below a floor
- **Wage-free / cooperative** — distribute near-zero-cost goods at little/no charge
Tags: economics ai labor
END

START
Coding Questions
Why is UBI framed as **economics, not charity** — and what does it *not* fix?
Back: It keeps **aggregate demand** alive so the productive machine has a reason to run.
- Open gap: UBI sustains **consumption** but does nothing about the **power** concentrated in whoever owns the AGI
Tags: economics ai labor
END

START
Coding Questions
What is **"vibe reporting"** (Cal Newport)?
Back: Journalism that builds a **predetermined narrative through omission and suggestion**, never making a checkable claim.
- Implies *"AI is taking jobs"* without asserting it → can't be fact-checked
- A Deutsche Bank analyst mocked one piece's *"vibes-to-substance ratio"* as *"undeniably high"*
Tags: economics ai labor
END

START
Coding Questions
What three **moves** does vibe reporting use?
Back: - **Omits** the publicly stated reason for a layoff
- **Suggestive placement** — puts AI quotes next to layoff news to imply causation
- **Appeals to biased authorities** — quotes AI CEOs who profit from sounding world-altering
Tags: economics ai labor
END

START
Coding Questions
What is the better-supported explanation for the 2024–2025 **tech layoffs**?
Back: A **pandemic hiring bubble correcting** — not AI automation.
- Firms over-hired for pandemic digital demand; cuts reverse that, plus margin pressure and bloat
- IBM CEO **Krishna** attributed the wave to over-hiring, not AI
Tags: economics ai labor
END

START
Coding Questions
What **timing** fact undercuts the "AI caused the layoffs" claim?
Back: Amazon's cuts began in **2022–2023, before generative AI was deployable at scale**, aimed at *trimming management bureaucracy*.
- A cause can't post-date its effect
- Caveat: the **2025 wave** shifts toward strategic restructuring, some genuinely AI-related; "AI" is also a convenient cover story
Tags: economics ai labor
END

START
Coding Questions
Why discount an **AI CEO's** prediction that AI will replace the workforce?
Back: It is **marketing, not forecasting** — they're the most conflicted source.
- Hype justifies **high valuations** and massive compute spend, and deflects scrutiny of sustainability
- Newport's analogy: a Pfizer exec touting a pill that *cures psoriasis but turns half the population into zombies* — as a selling point
Tags: economics ai labor
END

START
Coding Questions
Why does technology follow an **S-curve** rather than a runaway exponential (Narayanan & Kapoor)?
Back: Pace is set by **adoption (diffusion + adaptation)**, not by the speed of invention.
- Stages: invention → product → **diffusion → adaptation**
- Electricity and the internet took **decades**; even now <7% adopt advanced reasoning models — a **behavioral**, not capability, limit
Tags: economics ai labor
END

START
Coding Questions
How do **physical compute constraints** cap AI automation — and which two effects do they have?
Back: GPU supply, data-center capacity, and grid power are **hard physical ceilings** on deployment.
- **Slows** mass automation (tempers doom)
- **Forces prioritization** — scarce compute goes to high-value bottleneck work, leaving much labor untouched
Tags: economics ai labor
END

START
Coding Questions
What does framing AI as a **"normal technology"** imply for accountability?
Back: AI is a **general-purpose technology** (like electricity/internet), so its diffusion is **shaped by human institutions** — not inevitable.
- Inevitability framing lets companies off the hook for *how* they deploy AI
- "Normal" framing → use **regulation and labor negotiation**, not panic
Tags: economics ai labor
END

START
Coding Questions
What 2026 **evidence** argues against an imminent AI-driven collapse?
Back: - Labor market is in a **"low hire, low fire" freeze**, not a spiral (unemployment ~4.4%, forecast ~4.5% by end-2026)
- **NY Fed:** *"little indication of a distinct AI-driven decline in labor demand"* — high-exposure postings diverged **before** ChatGPT; entry-level vs senior move **in parallel**
- This **tempers** the Stanford "canaries" reading
Tags: economics ai labor
END
